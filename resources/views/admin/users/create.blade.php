...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-600 mb-2">
            ...
            <span>/</span>
            <span class="text-slate-900">Tambah User</span>
        </div>
    </div>
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="role" id="role" required
                        ...
                        <option value="staf_desa" {{ old('role') == 'staf_desa' ? 'selected' : '' }}>Staf Desa</option>
                        <option value="perwakilan_masyarakat" {{ old('role') == 'perwakilan_masyarakat' ? 'selected' : '' }}>
                            Perwakilan Masyarakat</option>
                    </select>
                    @error('role')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
