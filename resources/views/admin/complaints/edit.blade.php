...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-600 mb-2">
            <a href="{{ route('admin.complaints.index') }}" class="hover:text-slate-900">Kelola Pengaduan</a>
            <span>/</span>
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-8">
            <form action="{{ route('admin.complaints.update', $complaint) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Assigned To Field -->
                    <div>
                        <label for="assigned_to" class="block text-sm font-medium text-slate-700 mb-2">Ditugaskan
                            Kepada</label>
                        <select name="assigned_to" id="assigned_to"
                            class="block w-full px-4 py-2.5 bg-white border {{ $errors->has('assigned_to') ? 'border-red-300' : 'border-slate-300' }} rounded-lg text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                            <option value="">Tidak Ditugaskan</option>
                            @foreach($users as $user)
                                @if($user->role !== 'perwakilan_masyarakat')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-slate-500">Format: jpeg, png, jpg, pdf, doc, docx. Maksimal 2MB.
                            Kosongkan jika tidak ingin diubah.</p>
