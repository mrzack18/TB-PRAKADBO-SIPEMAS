...
@section('content')
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-slate-600 mb-2">
            <a href="{{ route('admin.news.index') }}" class="hover:text-slate-900">Kelola Berita</a>
            <span>/</span>
    ...
    <div class="max-w-4xl">
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div class="px-8 py-6 border-b border-slate-200">
                <h3 class="text-2xl font-bold text-slate-900">{{ $news->title }}</h3>
            </div>
            ...
            <div class="px-8 py-6">
                ...
                <div class="grid grid-cols-2 gap-x-6 gap-y-4 mb-6 pb-6 border-b border-slate-200">
                    <div>
                        <p class="text-sm font-medium text-slate-500 mb-1">Penulis</p>
                        <p class="text-base text-slate-900">{{ $news->user->name }}</p>
                    </div>
        ...
        ($news->status === 'draft' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-700') }}">
                            {{ ucfirst($news->status) }}
                        </span>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-base text-slate-900">{{ $news->created_at->format('d M Y H:i') }}</p>
                    </div>

                    <div>
                        <p class="text-sm font-medium text-slate-500 mb-1">Diupdate Pada</p>
                        <p class="text-base text-slate-900">{{ $news->updated_at->format('d M Y H:i') }}</p>
                    </div>
                </div>

                <!-- Content -->
                <div>
                    <h4 class="text-lg font-semibold text-slate-900 mb-3">Konten</h4>
                ...
                <div class="mt-12 pt-8 border-t border-slate-200">
                    ...
                    <div class="bg-slate-50 rounded-xl p-6 mb-10 border border-slate-200">
                        <form action="{{ route('comments.store') }}" method="POST">
                            ...
                            <div class="mb-4">
                                <label for="content" class="block text-sm font-medium text-slate-700 mb-2">Tulis Komentar</label>
